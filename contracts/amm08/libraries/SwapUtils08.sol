...
import "./AmplificationUtils08.sol";
import "./MathUtils08.sol";
import "../LPToken08.sol";
...
library SwapUtils08 {
    ...
    function _getAPrecise(Swap storage self) internal view returns (uint256) {
        return AmplificationUtils08._getAPrecise(self);
    }
    ...
    function getYD(
        ...
        c = (c * d * AmplificationUtils08.A_PRECISION) / (nA * numTokens);
        // c = c.mul(d).mul(AmplificationUtils08.A_PRECISION).div(nA.mul(numTokens));

        uint256 b = s + (d * AmplificationUtils08.A_PRECISION) / nA;
        // uint256 b = s.add(d.mul(AmplificationUtils08.A_PRECISION).div(nA));
        uint256 yPrev;
        uint256 y = d;
    ...
    function getD(uint256[] memory xp, uint256 a) internal pure returns (uint256) {
        ...
        for (uint256 i = 0; i < MAX_LOOP_LIMIT; i++) {
            ...
            prevD = d;
            d =
                (((nA * s) / AmplificationUtils08.A_PRECISION + (dP * numTokens)) * d) /
                (((nA - AmplificationUtils08.A_PRECISION) * d) /
                    AmplificationUtils08.A_PRECISION +
                    (numTokens + 1) *
                    dP);

            // d = nA
            //     .mul(s)
            //     .div(AmplificationUtils08.A_PRECISION)
            //     .add(dP.mul(numTokens))
            //     .mul(d)
            //     .div(
            //         nA
            //             .sub(AmplificationUtils08.A_PRECISION)
            //             .mul(d)
            //             .div(AmplificationUtils08.A_PRECISION)
    ...
    function getVirtualPrice(Swap storage self) external view returns (uint256) {
        ...
        LPToken08 lpToken = self.lpToken;
    ...
    function removeLiquidityOneToken(
        ...
        self.balances[tokenIndex] = self.balances[tokenIndex] - (dy + (dyFee * self.adminFee) / FEE_DENOMINATOR);
