...
contract SynFraxAdapter is WrapperAdapter {
    ...
    function _isPaused() internal view virtual override returns (bool) {
        return IFrax(tokenNative).exchangesPaused() || !IFrax(tokenNative).canSwap(tokenWrapped);
    }
    ...
    {
        _amountOut = IFrax(tokenNative).exchangeCanonicalForOld(tokenWrapped, _amountIn);
    ...
    function _swapWrappedToNative(uint256 _amountIn, address _to)
        ...
        returns (uint256 _amountOut)
    {
        _amountOut = IFrax(tokenNative).exchangeOldForCanonical(tokenWrapped, _amountIn);
        _returnTo(tokenNative, _amountOut, _to);
    }

    function _queryNativeToWrapped(uint256 _amountIn) internal view virtual override returns (uint256 _amountOut) {
        _amountOut = _amountIn;
        if (!IFrax(tokenNative).fee_exempt_list(address(this))) {
            _amountOut -= (_amountOut * IFrax(tokenNative).swap_fees(tokenWrapped, 1)) / PRICE_PRECISION;
        }
    ...
    function _queryWrappedToNative(uint256 _amountIn) internal view virtual override returns (uint256 _amountOut) {
        _amountOut = _amountIn;
        if (!IFrax(tokenNative).fee_exempt_list(address(this))) {
            _amountOut -= (_amountOut * IFrax(tokenNative).swap_fees(tokenWrapped, 0)) / PRICE_PRECISION;
        }
        uint256 _newTotalSupply = IERC20(tokenNative).totalSupply() + _amountOut;
