...
contract HeroBridge is SynMessagingReceiver {
    ...
    function _createMessage(uint256 _heroId, address _dstUserAddress, Hero memory _heroToBridge) internal pure returns (bytes memory) {
    ...
    function sendHero(uint256 _heroId, uint256 _dstChainId) external payable {
        ...
        bytes memory msgToPass = _createMessage(_heroId, msg.sender, heroToBridge);
        // Create _options
        // temporarily empty
