...
describe("MiniChefV21", function () {
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.MiniChefV21, [this.synapse.address]],
  ...
  describe("PendingSynapse", function () {
    it("PendingSynapse should equal ExpectedSynapse", async function () {
      ...
      let timestamp = (await ethers.provider.getBlock(log.blockNumber))
        .timestamp
      let expectedSynapse = BigNumber.from("10000000000000000").mul(
  ...
  describe("Harvest", function () {
    ...
    it("Harvest for SYNAPSE-only pool", async function () {
      ...
      await this.chef.harvest(0, this.alice.address)
      expect(await this.synapse.balanceOf(this.alice.address)).to.be.equal(
        expectedSynapse,
      )
    })
  })
