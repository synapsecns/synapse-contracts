...
describe("Swap08", async function () {
  ...
  describe("addLiquidity", () => {
    it("Reverts when contract is paused", async function () {
      ...
      await expect(swap.connect(user1).addLiquidity([String(1e18), String(3e18)], 0, MAX_UINT256))
        .to.be.reverted;
    ...
    it("Reverts with 'Cannot withdraw more than available'", async function () {
      await expect(
        swap.connect(user1).calculateTokenAmount([MAX_UINT256, String(3e18)], false)
    ...
    it("Reverts with 'Must supply all tokens in pool'", async function () {
      ...
      await expect(
        swap.connect(user1).addLiquidity([0, String(3e18)], MAX_UINT256, MAX_UINT256)
  ...
  describe("removeLiquidity", () => {
    it("Reverts with 'Cannot exceed total supply'", async function () {
      await expect(swap.calculateRemoveLiquidity(MAX_UINT256)).to.be.revertedWith(
    ...
    it("Succeeds with expected return amounts of underlying tokens", async function () {
      ...
      const [firstTokenBalanceBefore, secondTokenBalanceBefore, poolTokenBalanceBefore] =
        await getUserTokenBalances(user1, [firstToken, secondToken, swapToken]);
    ...
    it("Reverts when user tries to burn more LP tokens than they own", async function () {
      ...
      await expect(
        swap
          .connect(user1)
          .removeLiquidity(currentUser1Balance.add(1), [MAX_UINT256, MAX_UINT256], MAX_UINT256)
      ).to.be.reverted;
    });
    ...
    it("Emits removeLiquidity event", async function () {
      ...
      await swapToken.connect(user1).approve(swap.address, currentUser1Balance);
      await expect(
        swap.connect(user1).removeLiquidity(currentUser1Balance, [0, 0], MAX_UINT256)
  ...
  describe("removeLiquidityImbalance", () => {
    it("Reverts when contract is paused", async function () {
      ...
      await expect(
        swap.removeLiquidityImbalance([String(1e18), String(1e16)], MAX_UINT256, MAX_UINT256)
      ).to.be.reverted;

      await expect(
        swap
          .connect(user1)
          .removeLiquidityImbalance([String(1e18), String(1e16)], MAX_UINT256, MAX_UINT256)
      ).to.be.reverted;
    });
    ...
    it("Reverts with 'Cannot withdraw more than available'", async function () {
      await expect(
        swap.removeLiquidityImbalance([MAX_UINT256, MAX_UINT256], 1, MAX_UINT256)
      ).to.be.revertedWith("Cannot withdraw more than available");
    });
    ...
    it("Succeeds with calculated max amount of pool token to be burned (±0.1%)", async function () {
      ...
      const [firstTokenBalanceAfter, secondTokenBalanceAfter, poolTokenBalanceAfter] =
        await getUserTokenBalances(user1, [firstToken, secondToken, swapToken]);
  ...
  describe("removeLiquidityOneToken", () => {
    it("Reverts when contract is paused.", async function () {
      ...
      await expect(
        swap.connect(user1).removeLiquidityOneToken(currentUser1Balance, 0, 0, MAX_UINT256)
      ).to.be.reverted;
    });

    it("Reverts with 'Token index out of range'", async function () {
      await expect(swap.calculateRemoveLiquidityOneToken(1, 5)).to.be.revertedWith(
    ...
    it("Reverts when user tries to burn more LP tokens than they own", async function () {
      ...
      await expect(
        swap.connect(user1).removeLiquidityOneToken(currentUser1Balance.add(1), 0, 0, MAX_UINT256)
      ).to.be.reverted;
    });
  ...
  describe("swap", () => {
    ...
    it("Succeeds with expected swap amounts", async function () {
      ...
      const [tokenFromBalanceAfter, tokenToBalanceAfter] = await getUserTokenBalances(user1, [
        firstToken,
        secondToken,
      ]);
    ...
    it("Reverts when minDy (minimum amount token to receive) is not reached due to front running", async function () {
      ...
      await expect(swap.connect(user1).swap(0, 1, String(1e17), calculatedSwapReturn, MAX_UINT256))
        .to.be.reverted;
    });
    ...
    it("Reverts when block is mined after deadline", async function () {
      ...
      await expect(
        swap.connect(user1).swap(0, 1, String(1e17), 0, currentTimestamp + 60 * 5)
      ).to.be.revertedWith("Deadline not met");
    });

    it("Emits TokenSwap event", async function () {
      // User 1 initiates swap
      await expect(swap.connect(user1).swap(0, 1, String(1e17), 0, MAX_UINT256)).to.emit(
        swap,
        "TokenSwap"
      );
    });
  ...
  describe("withdrawAdminFees", () => {
    ...
    it("Succeeds when there are no fees withdrawn", async function () {
      ...
      const [firstTokenBefore, secondTokenBefore] = await getUserTokenBalances(owner, [
        firstToken,
        secondToken,
      ]);
      ...
      const [firstTokenAfter, secondTokenAfter] = await getUserTokenBalances(owner, [
        firstToken,
        secondToken,
      ]);
    ...
    it("Withdrawing admin fees has no impact on users' withdrawal", async function () {
      ...
      const [firstTokenBefore, secondTokenBefore] = await getUserTokenBalances(user1, [
        firstToken,
        secondToken,
      ]);
      ...
      const [firstTokenAfter, secondTokenAfter] = await getUserTokenBalances(user1, [
        firstToken,
        secondToken,
      ]);
  ...
  describe("rampA", () => {
    ...
    it("Emits RampA event", async function () {
      await expect(
        swap.rampA(100, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)
      ).to.emit(swap, "RampA");
    });
    ...
    it("Reverts when non-owner calls it", async function () {
      await expect(
        swap.connect(user1).rampA(55, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)
      ).to.be.reverted;
    });
    ...
    it("Reverts with 'futureA_ is too large'", async function () {
      await expect(
        swap.rampA(101, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)
    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      describe(
        ...
        () => {
          it("Attack fails with 900 seconds between blocks", async function () {
            // Swap08 1e18 of firstToken to secondToken, causing massive imbalance in the pool
            ...
            const firstTokenOutput = (await getUserTokenBalance(attacker, firstToken)).sub(
              balanceBefore
            );
            ...
            expect(initialAttackerBalances[0].sub(finalAttackerBalances[0])).to.be.eq(
              "2785303425594263"
            );
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            // Swap08 1e18 of firstToken to secondToken, resolving imbalance in the pool
            await swap.connect(attacker).swap(0, 1, String(1e18), 0, MAX_UINT256);
            const secondTokenOutput = (await getUserTokenBalance(attacker, secondToken)).sub(
              initialAttackerBalances[1]
            );
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async function () {
            ...
            // Swap08 1e18 of firstToken to secondToken, resolving the imbalance in the pool
            await swap.connect(attacker).swap(0, 1, String(1e18), 0, MAX_UINT256);
            const secondTokenOutput = (await getUserTokenBalance(attacker, secondToken)).sub(
              initialAttackerBalances[1]
            );
            ...
            expect(finalAttackerBalances[0].sub(initialAttackerBalances[0])).to.be.eq(
              "4298818514364451"
            );
