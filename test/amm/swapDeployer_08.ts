...
import { SwapDeployer08 } from "../../build/typechain/SwapDeployer08";
import { SwapUtils08 } from "../../build/typechain/SwapUtils08";
...
describe("Swap08 Deployer", () => {
  let signers: Array<Signer>;
  let swap: Swap08;
  let swapClone: Swap08;
  let swapDeployer: SwapDeployer08;
  let swapUtils: SwapUtils08;
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }) => {
    ...
    const swapDeployerFactory = await ethers.getContractFactory("SwapDeployer08");
    swapDeployer = (await swapDeployerFactory.deploy()) as SwapDeployer08;
    ...
    const swapCloneAddress = await swapDeployer.callStatic.deploy(
      ...
      0,
      (
        await deployments.get("LPToken08")
      ).address
    );
    ...
    swapClone = (await ethers.getContractAt("Swap08", swapCloneAddress)) as Swap08;
  ...
  describe("addLiquidity", () => {
    it("Add liquidity succeeds with pool with 4 tokens", async function () {
      const calcTokenAmount = await swapClone.calculateTokenAmount([String(1e18), 0, 0, 0], true);
  ...
  describe("swap", () => {
    it("Swap08 works between tokens with different decimals", async function () {
      const calcTokenAmount = await swapClone.connect(user1).calculateSwap(2, 0, String(1e6));
  ...
  describe("removeLiquidity", () => {
    it("Remove Liquidity succeeds", async function () {
      ...
      const expectedAmounts = await swapClone.calculateRemoveLiquidity("999355335447632820");
  ...
  describe("Check for timestamp manipulations", () => {
    ...
    it("Check for maximum differences in A and virtual price when increasing", async function () {
      ...
      await swapClone.connect(user1).addLiquidity([String(1e20), 0, 0, 0], 0, MAX_UINT256);

      // Start ramp
      await swapClone.rampA(100, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1);
    ...
    it("Check for maximum differences in A and virtual price when decreasing", async function () {
      ...
      // Start ramp
      await swapClone.rampA(25, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1);
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            ...
            await swapClone.connect(attacker).swap(1, 3, String(16e6), 0, MAX_UINT256);
            ...
            expect(await swapClone.getTokenBalance(3)).to.be.eq("34126363338064619373");
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            ...
            await swapClone.connect(attacker).swap(1, 3, String(25e6), 0, MAX_UINT256);
            const SUSDOutput = (await getUserTokenBalance(attacker, SUSD)).sub(
              initialAttackerBalances[3]
            );
            ...
            expect(await swapClone.getTokenBalance(3)).to.be.eq("74859519956589418582");
