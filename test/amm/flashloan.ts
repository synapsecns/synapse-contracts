...
describe("Swap Flashloan", () => {
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }) => {
    ...
    const flashLoanExampleFactory = await ethers.getContractFactory("FlashLoanBorrowerExample");
  ...
  it("Succeeds when fee is paid off", async function () {
    ...
    const flashLoanFee = flashLoanAmount.mul(await swapFlashLoan.flashLoanFeeBPS()).div(10000);
    ...
    await expect(flashLoanExample.flashLoan(swapFlashLoan.address, USDC.address, 1e6, [])).to.emit(
      swapFlashLoan,
      "FlashLoan"
    );
  ...
  describe("setFlashLoanFees", () => {
    it("Reverts when called by non-owner", async function () {
      await expect(swapFlashLoan.connect(user1).setFlashLoanFees(100, 5000)).to.be.revertedWith(
    ...
    it("Reverts when fees are not in the range", async function () {
      ...
      await expect(swapFlashLoan.setFlashLoanFees(100000, 5000)).to.be.revertedWith(
        "fees are not in valid range"
      );
      await expect(swapFlashLoan.setFlashLoanFees(100, 100000)).to.be.revertedWith(
        "fees are not in valid range"
      );
      await expect(swapFlashLoan.setFlashLoanFees(100000, 100000)).to.be.revertedWith(
        "fees are not in valid range"
      );
    });
    ...
    it("Succeeds when protocol fee bps is set to 0", async function () {
      ...
      await swapFlashLoan.setFlashLoanFees(newFlashLoanFeeBPS, newProtocolFeeBPS);
      ...
      // Since the contract is empty, we need to give the contract some USDC to have enough to pay off the fee
      await USDC.connect(user1).transfer(flashLoanExample.address, flashLoanFee);
      await flashLoanExample.flashLoan(swapFlashLoan.address, USDC.address, 1e6, []);
