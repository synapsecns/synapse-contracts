...
describe("SwapCalculator 0.6", async function () {
  ...
  async function testAddLiquidity(multipliers: Array<number>) {
    ...
    for (let i in AMOUNTS) {
      ...
      for (let j in unbalanced) {
        ...
        let quotedDeposit = await swapAddCalculator.calculateAddLiquidity(depositAmounts);
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }) => {
    ...
    const swapCalculatorFactory = await ethers.getContractFactory(
      "contracts/amm/SwapCalculator.sol:SwapCalculator"
    );

    swapAddCalculator = (await swapCalculatorFactory.deploy(swap.address)) as SwapAddCalculator;
  });
  ...
  describe("Adding to empty pool", () => {
    it("Reverts when providing not all tokens into empty pool", async function () {
      ...
      for (let i in AMOUNTS) {
        ...
        await expect(swapAddCalculator.calculateAddLiquidity(depositAmounts)).to.be.revertedWith(
    ...
    it("Returns deposited value when providing all tokens into empty pool", async function () {
      for (let i in AMOUNTS) {
        ...
        expect(await swapAddCalculator.calculateAddLiquidity(depositAmounts)).to.be.eq(
          getBigNumber(total)
        );
      }
    });
  ...
  describe("Adding to existing pool", () => {
    ...
    it("Reverts when quoting empty deposit", async function () {
      await expect(swapAddCalculator.calculateAddLiquidity([0, 0, 0, 0])).to.be.revertedWith(
        "D should increase"
      );
    });
    ...
    it("New fee is applied after updateSwapFee", async function () {
      ...
      let oldQuotedDeposit = await swapAddCalculator.calculateAddLiquidity(depositAmounts);
