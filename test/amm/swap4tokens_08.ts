...
import { LPToken08 } from "../../build/typechain/LPToken08";
import { Swap08 } from "../../build/typechain/Swap08";
import chai from "chai";

chai.use(solidity);
const { expect } = chai;

describe("Swap08 with 4 tokens", () => {
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }) => {
    ...
    // Deploy Swap08 with SwapUtils08 library
    const swapFactory = await ethers.getContractFactory("Swap08", {
      libraries: {
        SwapUtils08: (await get("SwapUtils08")).address,
        AmplificationUtils08: (await get("AmplificationUtils08")).address,
      },
    });
    swap = (await swapFactory.deploy()) as Swap08;
    ...
    swapToken = (await ethers.getContractAt("LPToken08", swapStorage.lpToken)) as LPToken08;
  ...
  describe("addLiquidity", () => {
    it("Add liquidity succeeds with pool with 4 tokens", async function () {
      const calcTokenAmount = await swap.calculateTokenAmount([String(1e18), 0, 0, 0], true);
      ...
      expect(await swapToken.balanceOf(await user1.getAddress())).to.be.eq("999355335447632820");
    });
  });

  describe("swap", () => {
    it("Swap08 works between tokens with different decimals", async function () {
      const calcTokenAmount = await swap.connect(user1).calculateSwap(2, 0, String(1e6));
      ...
      await swap
        .connect(user1)
        .swap(2, 0, String(1e6), calcTokenAmount, (await getCurrentBlockTimestamp()) + 60);
  ...
  describe("removeLiquidity", () => {
    it("Remove Liquidity succeeds", async function () {
      ...
      const expectedAmounts = await swap.calculateRemoveLiquidity("999355335447632820");
      ...
      expect(afterTokenBalances[0].sub(beforeTokenBalances[0])).to.be.eq("253568584947798923");
      expect(afterTokenBalances[1].sub(beforeTokenBalances[1])).to.be.eq("248596");
      expect(afterTokenBalances[2].sub(beforeTokenBalances[2])).to.be.eq("248596");
      expect(afterTokenBalances[3].sub(beforeTokenBalances[3])).to.be.eq("248596651909606787");
    });
  });
  ...
  describe("withdrawAdminFees", () => {
    ...
    it("Succeeds when there are no fees withdrawn", async function () {
      ...
      const balancesBefore = await getUserTokenBalances(owner, [DAI, USDC, USDT, SUSD]);

      await swap.withdrawAdminFees();

      const balancesAfter = await getUserTokenBalances(owner, [DAI, USDC, USDT, SUSD]);
    ...
    it("Succeeds with expected amount of fees withdrawn", async function () {
      ...
      expect(balancesAfter[0].sub(balancesBefore[0])).to.eq(String(10003917589952));
    ...
    it("Withdrawing admin fees has no impact on users' withdrawal", async function () {
      ...
      await swap
        .connect(user1)
        .addLiquidity([String(1e18), String(1e6), String(1e6), String(1e18)], 0, MAX_UINT256);
      ...
      const balancesBefore = await getUserTokenBalances(user1, [DAI, USDC, USDT, SUSD]);
      ...
      await swap.connect(user1).removeLiquidity(user1LPTokenBalance, [0, 0, 0, 0], MAX_UINT256);

      const balancesAfter = await getUserTokenBalances(user1, [DAI, USDC, USDT, SUSD]);

      expect(balancesAfter[0].sub(balancesBefore[0])).to.eq(BigNumber.from("1000119153497686425"));

      expect(balancesAfter[1].sub(balancesBefore[1])).to.eq(BigNumber.from("1000269"));
    });
  });
  ...
  describe("Check for timestamp manipulations", () => {
    it("Check for maximum differences in A and virtual price when increasing", async function () {
      ...
      await swap.connect(user1).addLiquidity([String(1e20), 0, 0, 0], 0, MAX_UINT256);
    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      beforeEach(async function () {
        ...
        // Start ramp upwards
        await swap.rampA(100, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 10);
        expect(await swap.getAPrecise()).to.be.eq(5000);
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            // Swap08 16e6 of USDC to SUSD, causing massive imbalance in the pool
            ...
            expect(await swap.getTokenBalance(3)).to.be.eq("34126363338064619373");
            ...
            const USDCOutput = (await getUserTokenBalance(attacker, USDC)).sub(balanceBefore);

            // If USDCOutput > 16e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("15967909");

            const finalAttackerBalances = await getUserTokenBalances(attacker, TOKENS);

            expect(finalAttackerBalances[1]).to.be.lt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(initialAttackerBalances[1].sub(finalAttackerBalances[1])).to.be.eq("32091");
            expect(initialAttackerBalances[3].sub(finalAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1].sub(initialPoolBalances[1])).to.be.eq("32091");
            expect(finalPoolBalances[3].sub(initialPoolBalances[3])).to.be.eq("0");
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async function () {
            ...
            expect(finalAttackerBalances[1]).to.be.lt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(initialAttackerBalances[1].sub(finalAttackerBalances[1])).to.be.eq("86512");
            expect(initialAttackerBalances[3].sub(finalAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1]).to.be.gt(initialPoolBalances[1]);
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(finalPoolBalances[1].sub(initialPoolBalances[1])).to.be.eq("86512");
            expect(finalPoolBalances[3].sub(initialPoolBalances[3])).to.be.eq("0");
      ...
      describe(
        ...
        () => {
          ...
          beforeEach(async function () {
            ...
            await swap
              .connect(user2)
              .addLiquidity([0, 0, 0, String(50e18)], 0, (await getCurrentBlockTimestamp()) + 60);
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            ...
            expect(await swap.getTokenBalance(3)).to.be.eq("74859519956589418582");
            ...
            expect(finalAttackerBalances[1]).to.be.lt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(initialAttackerBalances[1].sub(finalAttackerBalances[1])).to.be.eq("49826");
            expect(initialAttackerBalances[3].sub(finalAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1]).to.be.gt(initialPoolBalances[1]);
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(finalPoolBalances[1].sub(initialPoolBalances[1])).to.be.eq("49826");
            expect(finalPoolBalances[3].sub(initialPoolBalances[3])).to.be.eq("0");
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async function () {
            ...
            // Swap08 25e6 of USDC to SUSD, resolving the imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(25e6), 0, MAX_UINT256);
            const SUSDOutput = (await getUserTokenBalance(attacker, SUSD)).sub(
              initialAttackerBalances[3]
            );
            ...
            expect(initialAttackerBalances[1]).to.be.lt(finalAttackerBalances[1]);
            expect(initialAttackerBalances[3]).to.be.eq(finalAttackerBalances[3]);
            expect(finalAttackerBalances[1].sub(initialAttackerBalances[1])).to.be.eq("31387");
            expect(finalAttackerBalances[3].sub(initialAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(initialPoolBalances[1].sub(finalPoolBalances[1])).to.be.eq("31387");
            expect(initialPoolBalances[3].sub(finalPoolBalances[3])).to.be.eq("0");
    ...
    describe("Check for attacks while A is ramping downwards", () => {
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            ...
            expect(finalAttackerBalances[1]).to.be.lt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(initialAttackerBalances[1].sub(finalAttackerBalances[1])).to.be.eq("32005");
            expect(initialAttackerBalances[3].sub(finalAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1]).to.be.gt(initialPoolBalances[1]);
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(finalPoolBalances[1].sub(initialPoolBalances[1])).to.be.eq("32005");
            expect(finalPoolBalances[3].sub(initialPoolBalances[3])).to.be.eq("0");
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async function () {
            ...
            // Swap08 16e6 USDC to sUSD, causing imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(16e6), 0, MAX_UINT256);
            const SUSDOutput = (await getUserTokenBalance(attacker, SUSD)).sub(
              initialAttackerBalances[3]
            );
            ...
            // Check for attacker's balance changes
            expect(finalAttackerBalances[1]).to.be.gt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(finalAttackerBalances[1].sub(initialAttackerBalances[1])).to.be.eq("73391");
            expect(finalAttackerBalances[3].sub(initialAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1]).to.be.lt(initialPoolBalances[1]);
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(initialPoolBalances[1].sub(finalPoolBalances[1])).to.be.eq("73391");
            expect(initialPoolBalances[3].sub(finalPoolBalances[3])).to.be.eq("0");
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            // Swap08 25e6 of USDC to SUSD, resolving imbalance in the pool
            await swap.connect(attacker).swap(1, 3, String(25e6), 0, MAX_UINT256);
            const SUSDOutput = (await getUserTokenBalance(attacker, SUSD)).sub(
              initialAttackerBalances[3]
            );
            ...
            expect(finalAttackerBalances[1]).to.be.lt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(initialAttackerBalances[1].sub(finalAttackerBalances[1])).to.be.eq("49954");
            expect(initialAttackerBalances[3].sub(finalAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1]).to.be.gt(initialPoolBalances[1]);
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(finalPoolBalances[1].sub(initialPoolBalances[1])).to.be.eq("49954");
            expect(finalPoolBalances[3].sub(initialPoolBalances[3])).to.be.eq("0");
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async function () {
            ...
            expect(finalAttackerBalances[1]).to.be.lt(initialAttackerBalances[1]);
            expect(finalAttackerBalances[3]).to.be.eq(initialAttackerBalances[3]);
            expect(initialAttackerBalances[1].sub(finalAttackerBalances[1])).to.be.eq("205156");
            expect(initialAttackerBalances[3].sub(finalAttackerBalances[3])).to.be.eq("0");
            ...
            expect(finalPoolBalances[1]).to.be.gt(initialPoolBalances[1]);
            expect(finalPoolBalances[3]).to.be.eq(initialPoolBalances[3]);
            expect(finalPoolBalances[1].sub(initialPoolBalances[1])).to.be.eq("205156");
            expect(finalPoolBalances[3].sub(initialPoolBalances[3])).to.be.eq("0");
