...
describe("SwapCalculator 0.8", async function () {
  ...
  async function testAddLiquidity(multipliers: Array<number>) {
    ...
    for (let i in AMOUNTS) {
      ...
      for (let j in unbalanced) {
        let depositAmounts = [
          getBigNumber(unbalanced[(+j + 0) % unbalanced.length], TOKENS_DECIMALS[0]),
          getBigNumber(unbalanced[(+j + 1) % unbalanced.length], TOKENS_DECIMALS[1]),
          getBigNumber(unbalanced[(+j + 2) % unbalanced.length], TOKENS_DECIMALS[2]),
          getBigNumber(unbalanced[(+j + 3) % unbalanced.length], TOKENS_DECIMALS[3]),
        ];

        let quotedDeposit = await swapCalculator.calculateAddLiquidity(depositAmounts);

        await expect(() => swap.addLiquidity(depositAmounts, 0, MAX_UINT256)).to.changeTokenBalance(
          swapToken,
          owner,
          quotedDeposit
        );
  ...
  const setupTest = deployments.createFixture(async ({ deployments, ethers }) => {
    ...
    swapCalculator = (await swapCalculatorFactory.deploy(swap.address)) as SwapCalculator;
  });
  ...
  describe("Setup", () => {
    it("SwapCalculator is properly set up", async function () {
  ...
  describe("Adding to empty pool", () => {
    it("Reverts when providing not all tokens into empty pool", async function () {
      ...
      for (let i in AMOUNTS) {
        ...
        await expect(swapCalculator.calculateAddLiquidity(depositAmounts)).to.be.revertedWith(
    ...
    it("Returns deposited value when providing all tokens into empty pool", async function () {
      for (let i in AMOUNTS) {
        ...
        expect(await swapCalculator.calculateAddLiquidity(depositAmounts)).to.be.eq(
          getBigNumber(total)
        );
      }
    });
  ...
  describe("Adding to existing pool", () => {
    ...
    it("Reverts when quoting empty deposit", async function () {
      await expect(swapCalculator.calculateAddLiquidity([0, 0, 0, 0])).to.be.revertedWith(
        "D should increase"
      );
    });
    ...
    it("New fee is applied after updateSwapFee", async function () {
      ...
      let oldQuotedDeposit = await swapCalculator.calculateAddLiquidity(depositAmounts);
