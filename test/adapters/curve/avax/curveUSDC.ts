...
describe("Curve USDC pool (AVAX) Adapter", async () => {
  ...
  const AMOUNTS = [8, 1001, 96420, 420690]
  ...
  describe("Adapter Swaps", () => {
    it("Swaps between tokens [80 small-big swaps]", async () => {
      await testAdapter(curveBasePoolAdapter, ALL_TOKENS, ALL_TOKENS, 10)
    })
  })
})
