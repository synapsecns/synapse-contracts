...
describe(ADAPTER_NAME, async () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      await setupTokens(ownerAddress, config[CHAIN], tokenSymbols, amount)
  ...
  describe("Adapter Swaps", () => {
    it("Swaps between tokens [120 small-medium swaps]", async () => {
      await testAdapter(adapter, ALL_TOKENS, ALL_TOKENS, 2)
    })

    it("Swaps between tokens [120 big-ass swaps]", async () => {
      await testAdapter(adapter, ALL_TOKENS, ALL_TOKENS, 2, AMOUNTS_BIG)
    })
  })
})
