    ...
    describe("SynToken", () => {
        ...
        describe("Mint", () => {
            ...
            it("mintMultiple", async () => {
                await synapseERC20.connect(owner).grantRole(await synapseERC20.MINTER_ROLE(), await owner.getAddress());
                await synapseERC20.connect(owner).mintMultiple(user1Address, "1000", user2Address, "10");
                expect(await synapseERC20.connect(owner).balanceOf(user1Address)).to.be.eq("1000");
                expect(await synapseERC20.connect(owner).balanceOf(user2Address)).to.be.eq("10");
            });

            it("Mint not allowed without role", async () => {
                // await synapseERC20.connect(owner).grantRole(await synapseERC20.MINTER_ROLE(), await owner.getAddress());
                await expect(synapseERC20.connect(owner).mint(await user1.getAddress(), "1000")).to.be.reverted
