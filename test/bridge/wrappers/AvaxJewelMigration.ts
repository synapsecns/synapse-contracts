...
import { ERC20, SynapseERC20, AvaxJewelMigrationV2, SynapseBridge } from "../../../build/typechain";
...
describe("Avax Jewel Migration", async function () {
  ...
  before(async function () {
    ...
    synJewel = (await ethers.getContractAt("SynapseERC20", SYN_JEWEL)) as SynapseERC20;
    bridge = (await ethers.getContractAt("SynapseBridge", BRIDGE)) as SynapseBridge;
  });
  ...
  beforeEach(async function () {
    ...
    await asyncForEach([FAT_CAT, admin], async (address) => {
      await network.provider.send("hardhat_setBalance", [address, "0xFFFFFFFFFFFFFFFFFFFF"]);
    });
  ...
  describe("Sanity checks", function () {
    ...
    it("Migration reverts when minting rights are revoked", async function () {
      let minterRole = await synJewel.MINTER_ROLE();
      await synJewel.connect(adminSigner).revokeRole(minterRole, migrator.address);
    ...
    it("Only owner can withdraw legacy tokens", async function () {
      ...
      await migrator.connect(dude).migrateAndBridge(dudeAmount, dudeAddress, DFK_CHAIN_ID);
  ...
  describe("Migration", function () {
    it("Single-chain migration", async function () {
      ...
      expect(await multiJewel.balanceOf(ownerAddress)).to.eq(ownerBalance.sub(ownerAmount));

      await expect(() => migrator.connect(dude).migrate(dudeAmount)).to.changeTokenBalance(
        synJewel,
        dude,
        dudeAmount
      );
      expect(await multiJewel.balanceOf(dudeAddress)).to.eq(dudeBalance.sub(dudeAmount));
    });
    ...
    it("Cross-chain migration to DFK (Bridge event emitted)", async function () {
      ...
      await expect(migrator.migrateAndBridge(ownerAmount, ownerAddress, DFK_CHAIN_ID))
        .to.emit(bridge, "TokenRedeem")
        .withArgs(ownerAddress, DFK_CHAIN_ID, synJewel.address, ownerAmount);

      expect(await multiJewel.balanceOf(ownerAddress)).to.eq(ownerBalance.sub(ownerAmount));

      await expect(migrator.connect(dude).migrateAndBridge(dudeAmount, dudeAddress, DFK_CHAIN_ID))
        .to.emit(bridge, "TokenRedeem")
    ...
    it("Cross-chain migration to Harmony (Bridge event emitted)", async function () {
      ...
      await expect(migrator.migrateAndBridge(ownerAmount, ownerAddress, HARMONY_ID))
        .to.emit(bridge, "TokenRedeemAndSwap")
        .withArgs(ownerAddress, HARMONY_ID, synJewel.address, ownerAmount, 1, 0, 0, MAX_UINT256);

      expect(await multiJewel.balanceOf(ownerAddress)).to.eq(ownerBalance.sub(ownerAmount));

      await expect(migrator.connect(dude).migrateAndBridge(dudeAmount, dudeAddress, HARMONY_ID))
        .to.emit(bridge, "TokenRedeemAndSwap")
        .withArgs(dudeAddress, HARMONY_ID, synJewel.address, dudeAmount, 1, 0, 0, MAX_UINT256);
      expect(await multiJewel.balanceOf(dudeAddress)).to.eq(dudeBalance.sub(dudeAmount));
    });
  });
});
