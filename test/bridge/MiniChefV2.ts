...
describe("MiniChefV2", function () {
  before(async function () {
    await prepare(this, ['MiniChefV2', 'SynapseERC20', 'ERC20Mock', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    await deploy(this, [
      ["synapse", this.ERC20Mock, ["Dummy", "DummyT", getBigNumber(0)]],
    ])
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of SYNAPSE and reward", async function () {
        ...
        expect(await this.synapse.balanceOf(this.alice.address)).to.be.equal(await this.r.balanceOf(this.alice.address)).to.be.equal(expectedSynapse)
    })
