...
describe("Bridge Config V3", () => {
  ...
  describe("max gas", () => {
    ...
    it("should set max gas price for multiple chains", async () => {
      ...
      await expect(bridgeConfigV3.setMaxGasPrice(CHAIN_ID.BSC, bscGasPrice)).to.be.not.reverted;

      await expect(bridgeConfigV3.setMaxGasPrice(CHAIN_ID.MAINNET, mainnetGasPrice)).to.be.not
        .reverted;
    ...
    it("reverts when called by non-bridge managers", function () {
      expect(bridgeConfigV3.connect(attacker).setMaxGasPrice(CHAIN_ID.BSC, bscGasPrice)).to.be
        .reverted;
    });
  });
  ...
  describe("token config", () => {
    ...
    async function veriyTokenConfig(config: TokenConfigTest) {
      ...
      if (config.evmAddress) {
        ...
        let tokenIDByEvmAddress = await bridgeConfigV3["getTokenID(address,uint256)"](
          config.tokenAddress,
          config.chainID
        );
    ...
    it("should correctly calculate swap fees", async function () {
      ...
      const minFee = await bridgeConfigV3["calculateSwapFee(string,uint256,uint256)"](
        testToken.tokenAddress,
        testToken.chainID,
        BigNumber.from(4)
      );
      ...
      const testFee = await bridgeConfigV3["calculateSwapFee(address,uint256,uint256)"](
        testToken.tokenAddress,
        testToken.chainID,
      ...
      const maxFee = await bridgeConfigV3["calculateSwapFee(string,uint256,uint256)"](
        testToken.tokenAddress,
        testToken.chainID,
  ...
  describe("pool config", () => {
    ...
    it("reverts when called by non-bridge managers", function () {
      ...
      expect(
        bridgeConfigV3
          .connect(attacker)
          .setPoolConfig(config.tokenAddress, config.chainID, config.poolAddress, config.metaSwap)
      ).to.be.reverted;
    });
  });
});
