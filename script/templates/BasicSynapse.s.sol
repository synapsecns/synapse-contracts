...
contract BasicSynapseScript is BasicUtils {
    ...
    function create(string memory contractName, bytes memory constructorArgs) internal returns (address deployedAt) {
    ...
    /// Note: could be used as callback for `deployAndSave` and `deployAndSaveAs`.
    function create2(string memory contractName, bytes memory constructorArgs) internal returns (address deployedAt) {
