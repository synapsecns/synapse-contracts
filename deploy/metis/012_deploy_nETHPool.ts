...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (ETHPool) {
  ...
  } else if (!includes([CHAIN_ID.METIS, CHAIN_ID.HARDHAT], await getChainId())) {
    log(`Not Metis or Hardhat`);
  } else {
    // Constructor arguments
    const TOKEN_ADDRESSES = [(await get("nETH")).address, (await get("ETH")).address];
    ...
    if (currentOwner == deployer) {
      log(
        `transferring the ownership of "ETHPool" to the multisig: ${
      ...
      log(`"ETHPool" is already owned by the multisig: ${(await get("DevMultisig")).address}`);
    }
  }
};
export default func;
