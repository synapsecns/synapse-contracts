...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (nUSDPoolV3 || (await getChainId()) === CHAIN_ID.OPTIMISM || (await getChainId()) === CHAIN_ID.MAINNET || (await getChainId()) === CHAIN_ID.MOONBEAM || (await getChainId()) === CHAIN_ID.METIS || (await getChainId()) === CHAIN_ID.CRONOS) {
    ...
    if (currentOwner == deployer) {
      log(
        `transferring the ownership of "nUSDPoolV3" to the multisig: ${(await get("DevMultisig")).address}`,
      )
      await execute(
        "nUSDPoolV3",
        { from: deployer, log: true },
      ...
      log(
        `"nUSDPoolV3" is already owned by the multisig: ${(await get("DevMultisig")).address}`,
      )
    }
